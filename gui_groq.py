...
def client_detail(client_id: int):
    ...
    screening_data = {
        'screening_hit': 'YES',
        'hit_details': 'OSAMA BIN LADEN',
    }

    # Use dummy values for Agent Performance
    agent_data = {
        'researcher_time': '2h',
    ...
    def get_google_search_results(hit_details):
        return f"Latest news for {hit_details}: No recent articles found (placeholder)."
    ...
    with ui.element('div').classes('container mx-auto px-4'):
        ...
        with ui.element('div').classes('grid grid-cols-1 md:grid-cols-2 gap-6 mb-8'):
            with ui.card().classes('p-6 bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300'):
                ui.label('Onboarding Details').classes('text-2xl font-semibold text-gray-800 mb-4 border-b pb-2 border-gray-200')
                with ui.column().classes('gap-3'):
                    ...
                    ui.label(f"Client Specific Documents: {onboarding_data.get('document_name', 'N/A')}").classes('text-lg text-gray-600')

            with ui.card().classes('p-6 bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300'):
                ui.label('KYC Refresh Details').classes('text-2xl font-semibold text-gray-800 mb-4 border-b pb-2 border-gray-200')
                with ui.column().classes('gap-3'):
                    ...
                    ui.label(f"Document Name: {refresh_data.get('document_name', 'N/A')}").classes('text-lg text-gray-600')
                    ui.label(f"Country Issuing ID: {refresh_data.get('country_issuing_id', 'N/A')}").classes('text-lg text-gray-600')
                    ui.label(f"Refresh Status: {refresh_data.get('refresh_status', 'N/A')}").classes('text-lg text-gray-600')
                    ui.label(f"KYC Refresh Created Date: {refresh_data.get('onboarding_created_date', 'N/A')}").classes('text-lg text-gray-600')
                    ui.label(f"KYC Refresh Updated Date: {refresh_data.get('onboarding_updated_date', 'N/A')}").classes('text-lg text-gray-600')
        ...
        with ui.element('div').classes('grid grid-cols-1 lg:grid-cols-3 gap-6'):
            ...
            with ui.card().classes('p-6 bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300'):
                ...
                with ui.column().classes('gap-3'):
                    ui.label(f"Materiality Change: {refresh_data.get('materiality_change', 'NO')}").classes('text-lg text-gray-600 font-medium')
                    ui.label('Changed Values:').classes('text-lg font-semibold text-gray-600')
                    with ui.column().classes('gap-2 pl-4'):
                        for _, row in refresh_df.iterrows():
                            ui.label(f"Document: {row.get('document_name', 'N/A')}, Status: {row.get('refresh_status', 'N/A')}, "
                                     f"Updated: {str(row.get('onboarding_updated_date', 'N/A'))[:10]}").classes('text-sm text-gray-500')

            # Screening Card
            with ui.card().classes('p-6 bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300'):
                ui.label('Screening').classes('text-2xl font-semibold text-gray-800 mb-4 border-b pb-2 border-gray-200')
                with ui.column().classes('gap-3'):
                    ui.label(f"Screening Hit: {screening_data.get('screening_hit', 'NO')}").classes('text-lg text-gray-600 font-medium')
                    ui.label(f"Hit Details: {screening_data.get('hit_details', 'N/A')}").classes('text-lg text-gray-600')
                    if screening_data.get('screening_hit') == 'YES':
                        with ui.card().classes('p-4 bg-blue-50 rounded-lg shadow-sm border border-blue-100 mt-2'):
                            ui.label('Latest Google Search').classes('text-sm font-semibold text-gray-800 mb-2')
                            ui.label(get_google_search_results(screening_data.get('hit_details', 'Unknown'))).classes('text-sm text-gray-600')

            # Agents Performance Card
            with ui.card().classes('p-6 bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300'):
                ui.label('Agents Performance').classes('text-2xl font-semibold text-gray-800 mb-4 border-b pb-2 border-gray-200')
                with ui.column().classes('gap-3'):
                    ui.label('Researcher Agent:').classes('text-lg font-semibold text-gray-600')
                    with ui.column().classes('gap-1 pl-4'):
                        ui.label(f"Time Taken: {agent_data.get('researcher_time', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Accuracy: {agent_data.get('researcher_accuracy', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Tool Called: {agent_data.get('researcher_tool_called', 'NO')}").classes('text-sm text-gray-500')
                    ui.label('KYC Analyst Agent:').classes('text-lg font-semibold text-gray-600 mt-2')
                    with ui.column().classes('gap-1 pl-4'):
                        ui.label(f"Time Taken: {agent_data.get('kyc_analyst_time', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Accuracy: {agent_data.get('kyc_analyst_accuracy', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Tool Called: {agent_data.get('kyc_analyst_tool_called', 'NO')}").classes('text-sm text-gray-500')
                    ui.label('Screening Agent:').classes('text-lg font-semibold text-gray-600 mt-2')
                    with ui.column().classes('gap-1 pl-4'):
                        ui.label(f"Time Taken: {agent_data.get('screening_time', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Accuracy: {agent_data.get('screening_accuracy', 'N/A')}").classes('text-sm text-gray-500')
                        ui.label(f"Tool Called: {agent_data.get('screening_tool_called', 'NO')}").classes('text-sm text-gray-500')
